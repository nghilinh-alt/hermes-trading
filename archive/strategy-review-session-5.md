...
1. **Phase 2.1 (DONE, commit 55cbbaa)** — Three structural guards in one execution.py rewrite:
   - `max_sl_pct` (existing) — skip if SL > 5% from entry
   - `min_tp_pct: 3.0` (new) — skip if structural TP < 3% from entry (Option B filter)
   - `min_rr_ratio: 2.0` (new soft) — extend TP to hit 2:1 if structural R:R is thinner
2. **Phase 2.2 — Trailing stops (ATR-based)** — separate session. New `update_position_sl` + `fetch_open_positions_with_marks` in execution.py + new per-tick block in loop.py. Need ATR multiplier defaults; calibrate from real trade data after 2.1 has run a week.
3. **Phase 2.3 — Bybit-backfill for closed trades** — extends `fetch_last_closed_pnl` into `fetch_recent_closed_trades(asset, since_ts, limit=50)`. Dashboard reads merged history. Small session.
4. **Phase 2.4 — Reflection rebuild (NEW, added 2026-05-27)** — TAO has ~20 trades on Bybit but local `state/tao_usdt/trades.jsonl` shows 0 closed trades, so `_maybe_trigger_reflection` never crosses the cadence threshold. Two compounding causes:
   - **Bug A**: `reflect.py` has an `IndentationError` (uncommitted local edit from a prior session). When the subprocess is spawned, it fails with non-zero exit, never updating strategy.yaml. Visible in bot.log immediately after the per-asset loop boots.
   - **Bug B**: Trades that fired on Bybit *before* the per-asset state structure existed (and any trade where `_log_trade` failed silently) are not in `trades.jsonl`. `_count_closed_trades` only counts lines in that file; Bybit-side trade history is invisible to the reflection counter.
   - **Fix sequence in Phase 2.4**:
     a. Read current `reflect.py`, fix the IndentationError. Validate with `python -m py_compile`. Use Python heredoc (Edit/Write both corrupt files on the Windows mount).
     b. Backfill: write a one-shot `tools/backfill_trades.py` that calls Bybit's `private_get_v5_position_closed_pnl` per asset and appends missing trade records to `state/<slug>/trades.jsonl` (deduping by `order_id`). Run once on the VPS; never overwrites existing records.
     c. Verify: after backfill, `_count_closed_trades` for TAO should return ~20. The next post-trade tick will trigger reflection automatically. Watch `state/tao_usdt/strategy.yaml` version field flip from "01" to "02" or similar.
   - **Scope**: medium. Probably 90 minutes including the backfill tool and verification.
5. **Phase 2.5 — direction-aware pnl_pct in `fetch_last_closed_pnl`** — small fix; the current `(exit-entry)/entry` formula is wrong for shorts. Pair with 2.3 since both touch the same function.
