...
| 2026-07-03 (s12) | `position_pct` (10%) changed from a fixed value to a floor; scales up with confidence via new `position_pct_curve` | Linh's directive: minimum trade size = 10% of available margin ($80 at $800 balance), scaling up on stronger signals rather than staying fixed. New `_confidence_to_position_pct()` in execution.py interpolates over `position_pct_curve` (0.5→10%, 0.75→15%, 1.0→20%), clamped to never go below the base `position_pct`. Yamls without the curve key behave exactly as before. Leverage formula left untouched — still keyed off base `position_pct`, doesn't affect realized risk in this code's exposure model. Separate deploy doc, not bundled with the trend-filter fix. |
...
**Open question raised by Linh, not yet resolved**: the last XRP trade was ~$20, not the ~$80 the "10% of $800" example implies. Most likely explanation: the $800 figure in the sizing docstring was always illustrative (from the session-11 worked example), not the account's actual current free balance — 10% of a ~$200 actual balance would be exactly $20. Needs the actual XRP trade record + current free balance pulled from the VPS to confirm; flagged in chat, not yet checked against live data.
