...
- **VPS layout** (confirmed session 5b, 2026-05-27; old system's layout, still relevant for the archive/rollback):
  - **Git pull clone**: `/opt/trading/hermes-trading/` (hyphen) — deploy staging; now also holds the ICT Phase 1 code (synced session 18, not yet git-tracked there)
...
- VPS running code (old system, halted): `/opt/trading/hermes_trading/hermes_trading/` (nested — package inside package dir). Git repo on VPS: `/opt/trading/hermes-trading/` — now also holds ICT Phase 1+2+calibration code (synced via scp, not yet `git pull`-ed there).
