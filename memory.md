...
## Last Updated
2026-07-03 (session 12 — two fixes: (1) trade-frequency collapse — daily/4h trend filter AND-gate was hard-skipping BTC/XRP every tick for hours; converted 4h to a soft confidence discount. (2) position sizing bug — leverage was never actually applied to notional/exposure despite session 11's own docstring assuming it was; fixed to `notional = margin × leverage`, plus `position_pct` is now a confidence-scaled floor. Both implemented + validated locally; **pending VPS deploy by Linh**, two separate deploy docs, run one at a time)
...
**Trend-filter deploy in progress**: hyphen clone pulled clean (was stale at `36de5ee`, now at `f4746bc`). Backup step needed a fix — copying 4 `strategy.yaml` files with identical basenames into one directory in a single `cp` call clobbers itself; switched to 4 explicit named copies.
