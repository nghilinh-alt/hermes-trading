...
## Project Overview
**As of session 18 (2026-07-18): the old indicator-weight live agent is HALTED and fully archived.** Rebuilding as an ICT (Inner Circle Trader) swing strategy — see `ict-strategy-plan-2026-07-18.md` (spec) and `ict-claude-code-prompt.md` (build prompt/phases). Phase 1 (detection primitives), Phase 2 (backtester/setup/risk engine), and Phase 3 (alert-only scanner) are built, tested, and shipped — **the scanner runs live, unattended on the VPS as of session 19-20.** Phase 4+ (paper-trade position tracking, live worker, any order placement) not started.
...
- **Net state**: Phase 3 (alert-only scanner) is live and unattended on the VPS, continuously monitoring BTC/ETH/SOL/TAO for qualified ICT setups and logging them to JSONL. **Still NOT started**: Phase 4 (paper-trade position tracking) — deliberately deferred; the plan's explicit scope boundary keeps this session to alert-only since the underlying strategy edge is still only in-sample-validated (see extended-history entry above), not paper/live-forward-tested. No order placement exists anywhere in the new ICT codebase.
...
- VPS running code (old system, halted): `/opt/trading/hermes_trading/hermes_trading/` (nested — package inside package dir). Git repo on VPS: `/opt/trading/hermes-trading/` — properly git-tracked (session 19-20 fast-forward to `694c803`), now runs the live ICT Phase 3 scanner daemon in its own `.venv/`.
