...
- **ICT Phase 2**: built (setup/risk/backtest engine), 127 tests, 90% coverage, pushed to VPS staging clone + GitHub (commit `c7a8bc4`). Real 2-year BTC/ETH/SOL/TAO backtest run with locked spec defaults produced 0 closed trades — awaiting Linh's call on calibration vs. accepting as validated-but-conservative (see session 18 handoff).
...
- VPS running code (old system, halted): `/opt/trading/hermes_trading/hermes_trading/` (nested — package inside package dir). Git repo on VPS: `/opt/trading/hermes-trading/` — now also holds ICT Phase 1+2 code (synced via scp, not yet `git pull`-ed there).
...
- **Handoff to**: Linh — decide whether to (a) run a parameter calibration/walk-forward pass next (spec §14's own "prime calibration targets": score weights/thresholds, `state_ttl_bars`, `min_target_atr_mult` which is new this session, session windows), or (b) treat the mechanical logic itself as validated and move to Phase 3 (scanner, §12) once some parameter set produces a believable trade sample. Next agent: read this entry + `ict-strategy-plan-2026-07-18.md` §14 before touching any default parameter.
