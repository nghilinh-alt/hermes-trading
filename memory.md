...
- **Net position after both passes**: this loosened + bug-fixed config is the pushed reference point. Treat it as an **unvalidated, exploratory baseline, not a proven edge** — 2 of 4 assets (BTC, ETH) still have profit factor < 1, SOL is improved but still losing, only TAO is solid. The remaining fill-failure mode (63/233, invalidated on an earlier bar before ever reaching entry) reflects the core risk logic working as designed, not another graze bug — loosening that further would be a materially bigger quality tradeoff, not investigated this session.
...
- **Handoff to**: Linh / next agent — this calibrated config is now the live reference point but is explicitly **unvalidated** (an exploratory baseline optimized for frequency, not a proven edge) and **SOL's numbers are bad enough to warrant a closer look before this ever touches real money**. Open threads, any of which could meaningfully change the picture: (a) investigate the ~30% qualified→filled conversion rate — the conservative same-bar invalidation-priority rule in `_search_fill`/`_invalidated` may be losing fills unnecessarily; (b) SOL-specific investigation (liquidity/volatility profile) before using a universal parameter set across all 4 assets; (c) proper walk-forward/OOS split (this was one in-sample run over the full 2-year window, not validated out-of-sample) before Phase 3 (scanner) or any live/paper worker. Next agent: read this entry + `tools/run_ict_backtest.py`'s `CALIBRATED_PARAMS` docstring before changing any default parameter again.
